...
module tb_lms_pipelined_parity;
    ...
    lms_adaptive_filter_pipelined #(.MU_SHIFT(4)) dut (
        .clk(clk), .rst(rst),
    ...
    initial begin
        ...
        if (failures == 0) begin
            $display("PASS: staged parallel LMS matches 4096-sample bit-exact reference.");
            $display("Average accept-to-result latency: %0d clocks", total_latency / N);
            $display("Initiation interval: 5 clocks/sample");
            $finish;
        end else begin
            $display("FAIL: %0d pipelined parity mismatch(es).", failures);
            $fatal(1);
        end
    end
endmodule
